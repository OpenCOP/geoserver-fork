...
public class NetCDFParser {
    ...
    public static float[] getBoundsForFile(String file) {
        ...
        try {
            ...
            if (minAndMaxLon[MIN] < -180) {
                minAndMaxLon[MIN] = -180;
            }
            if (minAndMaxLon[MAX] > 180) {
                minAndMaxLon[MAX] = 180;
            }

            minAndMaxLat = getMinAndMaxFromArray(latArray);
            if (minAndMaxLat[MIN] < -90) {
                minAndMaxLat[MIN] = -90;
            }
            if (minAndMaxLat[MAX] > 90) {
                minAndMaxLat[MAX] = 90;
            }
        ...
        return new float[] { minAndMaxLon[MIN], minAndMaxLon[MAX], minAndMaxLat[MIN], minAndMaxLat[MAX] };
    }
