...
import org.geoserver.webeoc.BasicCredentials;
import org.geoserver.webeoc.WebEocDao;
import org.geoserver.webeoc.WebEocWebService;
...
public class WebEOCPollerTask extends TimerTask {
	...
	public void run() {
		...
		for (DataStoreInfo store : stores) {
			...
			Map<String, Serializable> connectionParameters = store
			...
			// Get the WebEOC WSDL endpoint from the datastore
			// connectionParameters map
			String wsdlUrl = connectionParameters.get(
					WebEOCConstants.WEBEOC_WSDL_KEY).toString();

			// Get all the layers for this WebEOC datastore
			List<FeatureTypeInfo> layers = catalog.getResourcesByStore(store,
					FeatureTypeInfo.class);
			for (FeatureTypeInfo featureType : layers) {
				// Get the WebEOC layer information from this featureType's
				// metadata
				// map.
				...
				if (webeoc.isPollingEnabled()) {
					System.out.println("Polling URL(" + wsdlUrl
							+ ") for layer: " + featureType.getName());
					...
					wsdlUrl = connectionParameters.get(
							WebEOCConstants.WEBEOC_WSDL_KEY).toString();

					WebEocWebService webEocWebService = new WebEocWebService(
							wsdlUrl);
					String webEOCXMLResponse = webEocWebService.getData(bcreds,
							incident, board, view);
